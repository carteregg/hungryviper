...
TEST(DeleteTest, MultiDeleteAndInvalidDelete)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	//Inserts items into the same bucket
	insertItem(ht, 0, m[0]);
	insertItem(ht, 3, m[1]);
	
	//Deletes items that should and shouldn't be 
	deleteItem(ht, 0);
	deleteItem(ht, 3);
	deleteItem(ht, 3);
	deleteItem(ht, 4);
...
TEST(InsertTest, InsertAsOverwrite)
{
	...
	EXPECT_EQ(m[1], insertItem(ht, 3, m[0]));
	...
	destroyHashTable(ht);    // don't forget to free item 0
	
}

TEST(InsertTest, MultiInsertRemovalHead)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	//Inserts multiple items into 2 separate buckets
	insertItem(ht, 0, m[0]);
	insertItem(ht, 1, m[1]);
	insertItem(ht, 3, m[2]);
	insertItem(ht, 4, m[3]);
	...
	//Removes the values in head first order 
	void *data = removeItem(ht, 1);
	EXPECT_EQ(m[1], data);
	data = removeItem(ht, 1);
	EXPECT_EQ(NULL, data);
	void *data2 = removeItem(ht, 4);
	EXPECT_EQ(m[3], data2);
	data2 = removeItem(ht, 4);
	EXPECT_EQ(NULL, data2);
	void *data3 = removeItem(ht, 7);
	EXPECT_EQ(m[5], data3);
	data3 = removeItem(ht, 7);
...
TEST(InsertTest, MultiInsertOverwriteTailFirst)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	//Inserts multiple items into a single bucket
	insertItem(ht, 1, m[0]);
	insertItem(ht, 4, m[1]);
	insertItem(ht, 7, m[2]);
	
	//Removes items in tail first order
	EXPECT_EQ(m[2], insertItem(ht, 7, m[3]));
	EXPECT_EQ(m[3], getItem(ht, 7));
	EXPECT_EQ(m[1], insertItem(ht, 4, m[4]));
	EXPECT_EQ(m[4], getItem(ht, 4));
	EXPECT_EQ(m[0], insertItem(ht, 1, m[5]));
	EXPECT_EQ(m[5], getItem(ht, 1));
